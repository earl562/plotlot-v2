...
for (const f of files) {
  ...
  rows.push({ arxivId, status, title, file: f });
}

rows.sort((a, b) => (a.status < b.status ? -1 : a.status > b.status ? 1 : a.arxivId.localeCompare(b.arxivId)));

const reviewed = rows.filter((r) => r.status === "reviewed");
const stub = rows.filter((r) => r.status === "stub");

const lines = [];
lines.push("# arXiv Notes Status\n");
...
for (const r of reviewed) {
  ...
  lines.push(`- [${r.arxivId}](${rel}) — ${r.title}`);
}
lines.push("");
