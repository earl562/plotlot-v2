...
/**
 * Municipality Coverage Tests — API-level validation that every ingested
 * municipality can produce a valid zoning report through the full pipeline.
 *
 * These tests hit the backend API directly (not the browser UI) for speed.
 * The browser-based UAT suite (uat.spec.ts) covers the full E2E UI flow.
 *
...
// ---------------------------------------------------------------------------
const MUNICIPALITY_ADDRESSES: Record<
  string,
...
// ---------------------------------------------------------------------------
// Discover which municipalities have ingested data
// ---------------------------------------------------------------------------
...
test.beforeAll(async ({ request }) => {
  ...
  // Get ingested municipalities
  const stats = await request.get(`${API}/api/v1/admin/chunks/stats`);
  ...
  console.log(
    `\nIngested: ${ingestedMunicipalities.size} municipalities | ` +
      `Testing: ${Object.keys(MUNICIPALITY_ADDRESSES).filter((m) => ingestedMunicipalities.has(m)).length} with data\n`,
  );
});

// ---------------------------------------------------------------------------
// Increase timeout — pipeline calls LLM + ArcGIS APIs
// ---------------------------------------------------------------------------
test.setTimeout(180_000);

// ---------------------------------------------------------------------------
// Generate one API-level test per municipality
// ---------------------------------------------------------------------------
for (const [municipality, { address, county }] of Object.entries(
  MUNICIPALITY_ADDRESSES,
)) {
  test(`${municipality} (${county}): ${address}`, async ({ request }) => {
    // Skip if no data for this municipality
    test.skip(
      !ingestedMunicipalities.has(municipality),
      `No data ingested for ${municipality}`,
    );

    // Call the synchronous analyze endpoint
    const response = await request.post(`${API}/api/v1/analyze`, {
      data: { address },
      timeout: 120_000,
    });

    // Handle expected error cases (geocoding failures, etc.)
    if (!response.ok()) {
      const err = await response.json().catch(() => ({ detail: "unknown" }));
      const detail = typeof err.detail === "string" ? err.detail : JSON.stringify(err.detail);
      // Known acceptable failures: geocoding accuracy, address not found
      if (detail.includes("accuracy") || detail.includes("geocode") || detail.includes("Could not")) {
        console.log(`  ${municipality}: SKIPPED — ${detail}`);
        test.skip(true, `Geocoding failed: ${detail}`);
      }
      // Unexpected failure
      expect(response.ok(), `${municipality} failed ${response.status()}: ${detail}`).toBeTruthy();
    }
    const report = await response.json();

    // ---- Core identification ----
    expect(report.municipality, "municipality should be set").toBeTruthy();
    expect(report.county, "county should be set").toBeTruthy();
    const normalize = (s: string) => s.toLowerCase().replace(/[-\s]/g, "");
    expect(normalize(report.county)).toContain(normalize(county));

    // ---- Zoning data ----
    expect(report.zoning_district, "zoning_district should be set").toBeTruthy();
    expect(report.summary, "summary should be non-empty").toBeTruthy();

    // ---- Dimensional standards ----
    expect(report.max_height || report.numeric_params?.max_height_ft, "max_height should be set").toBeTruthy();
    expect(report.max_density || report.numeric_params?.max_density_units_per_acre, "max_density should be set").toBeTruthy();

    // ---- Setbacks ----
    const setbacks = report.setbacks;
    expect(setbacks, "setbacks object should exist").toBeTruthy();

    // ---- Sources ----
    expect(Array.isArray(report.sources), "sources should be an array").toBe(true);

    // ---- Confidence ----
    expect(["high", "medium", "low"]).toContain(report.confidence);

    // ---- List fields are proper arrays (not JSON strings) ----
    expect(Array.isArray(report.allowed_uses), "allowed_uses should be array").toBe(true);
    expect(Array.isArray(report.conditional_uses), "conditional_uses should be array").toBe(true);
    expect(Array.isArray(report.prohibited_uses), "prohibited_uses should be array").toBe(true);

    // ---- Density analysis ----
    if (report.density_analysis) {
      expect(report.density_analysis.max_units, "max_units should be >= 0").toBeGreaterThanOrEqual(0);
      expect(Array.isArray(report.density_analysis.constraints), "constraints should be array").toBe(true);
    }

    // ---- Collect quality warnings (non-blocking) ----
    const warnings: string[] = [];
    if (report.sources.length === 0) warnings.push("NO SOURCES");
    if (!setbacks.front && !setbacks.side && !setbacks.rear) warnings.push("NO SETBACKS");
    if (report.property_record && report.property_record.lot_size_sqft === 0) warnings.push("LOT SIZE = 0");
    if (report.density_analysis && report.density_analysis.max_units === 0) warnings.push("MAX UNITS = 0");
    if (report.confidence === "low") warnings.push("LOW CONFIDENCE");

    // Log summary for test report
    console.log(
      `  ${municipality}: ${report.zoning_district} | ` +
        `${report.density_analysis ? `max ${report.density_analysis.max_units} unit(s)` : "no density"} | ` +
        `confidence: ${report.confidence} | ${report.sources.length} sources` +
